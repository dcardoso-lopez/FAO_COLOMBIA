...
# Filtro inicial de ejemplo
pob <- pob %>% dplyr::filter(DEPARTAMENTO_D == "ATLÁNTICO")
...
ui <- fluidPage(
  ...
  tags$head(
    tags$style(HTML("
      ...
      .info-quantiles-icon:hover{ color:#4bb5e1; }
      ...
      div(class="filters",
          div(class="filters-grid",
              ...
              div(class="filter", div(class="filter-label","¿En qué departamento?"),
                  selectInput("f_dep", NULL, choices = dep_choices, selected = "ATLÁNTICO")),
...
server <- function(input, output, session){
  ...
  # ================= ggplots base (reutilizables para plotly + descarga) =================
  gg_hist <- reactive({
    df <- serie_poblacion()
  ...
  output$dl_base_csv <- downloadHandler(
    ...
    content = function(file){
      utils::write.csv2(base_filtrada(), file, row.names = FALSE, fileEncoding = "UTF-8")
    }
  )
  
  # ================= Descarga PDF (render Informe_descargable.Rmd) =================
  output$dl_reporte_pdf <- downloadHandler(
    filename = function(){
      paste0("Informe_poblacion_", Sys.Date(), "_",
    ...
    content = function(file){
      ...
      # Carpeta real persistente para que el Rmd sí vea las imágenes
      app_root <- getwd()
      export_dir_name <- "Descargas"
      export_path <- file.path(app_root, export_dir_name)
      dir.create(export_path, recursive = TRUE, showWarnings = FALSE)
      
      # Guardar imágenes .png livianas en ./Descargas
      suppressWarnings({
      ...
      # Tabla de filtros para params$filtros
      ...
      # PDF de salida a temporal
      td <- tempfile("informe_pob_")
      dir.create(td, recursive = TRUE, showWarnings = FALSE)
      
      logo_file <- file.path(app_root, "www", "LOGO_PLATEA.png")
      if (!file.exists(logo_file)) {
        # fallback por si la carpeta está en mayúsculas
        logo_file2 <- file.path(app_root, "WWW", "LOGO_PLATEA.png")
        logo_file <- if (file.exists(logo_file2)) logo_file2 else NA_character_
      }
      ...
      out_pdf <- rmarkdown::render(
        ...
        params = list(
          ...
          logo_path    = if (!is.na(logo_file) && file.exists(logo_file)) logo_file else NULL
        ),
