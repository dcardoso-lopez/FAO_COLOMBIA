...
fmt_int   <- function(x) scales::number(x, big.mark = ".", decimal.mark = ",", accuracy = 1)
fmt_cop   <- function(x) paste0("$", scales::number(x, big.mark = ".", decimal.mark = ",", accuracy = 1))
fmt_mmilM <- function(x) paste0("$", scales::number(x / 1e9, big.mark = ".", decimal.mark = ",", accuracy = 0.1), " Mil M")
fmt_milM  <- function(x) paste0(scales::number(x / 1e9, big.mark = ".", decimal.mark = ",", accuracy = 0.1), " Mil M")
...
finagro_fast <- finagro_fast %>%
  dplyr::filter(NOM_DPTO == "ATLÁNTICO")
...
finagro_fast <- finagro_fast %>%
  dplyr::left_join(ipp_tbl, by = "ano") %>%
  dplyr::mutate(
...
server <- function(input, output, session) {
  ...
  observeEvent(input$depto_t1, {
    if (is.null(input$depto_t1) || input$depto_t1 == "Todos") {
      ...
      mpios_dep <- finagro_fast %>%
        dplyr::filter(NOM_DPTO == input$depto_t1) %>%
        dplyr::pull(NOM_MPIO) %>%
        unique() %>%
        sort()
  ...
  observeEvent(input$depto_t2, {
    if (is.null(input$depto_t2) || input$depto_t2 == "Todos") {
      ...
      mpios_dep <- finagro_fast %>%
        dplyr::filter(NOM_DPTO == input$depto_t2) %>%
        dplyr::pull(NOM_MPIO) %>%
        unique() %>%
        sort()
      mpio_choices_dep <- c("Todos" = "Todos", stats::setNames(mpios_dep, title_case_es(mpios_dep)))
  ...
  base_t1 <- reactive({
    df <- finagro_fast
    if (!is.null(input$ano_t1) && input$ano_t1 != "Todos") df <- df %>% dplyr::filter(ano == as.numeric(input$ano_t1))
    if (!is.null(input$depto_t1) && input$depto_t1 != "Todos") df <- df %>% dplyr::filter(NOM_DPTO == input$depto_t1)
    if (!is.null(input$mpio_t1) && input$mpio_t1 != "Todos")   df <- df %>% dplyr::filter(NOM_MPIO == input$mpio_t1)
    if (!is.null(input$eslabon_t1) && input$eslabon_t1 != "Todos") df <- df %>% dplyr::filter(ESLABON_TC == input$eslabon_t1)
    if (!is.null(input$productor_t1) && input$productor_t1 != "Todos") df <- df %>% dplyr::filter(TIPO_PRODUCTOR == input$productor_t1)
    df
  })
  ...
  output$hist_monto_txt <- renderUI({
    tags$div(class = "metric-value", fmt_mmilM(sum(base_t1()$VALOR_CREDITO_REAL, na.rm = TRUE)))
  })
  ...
  output$hist_mujeres_txt <- renderUI({
    ...
    tags$div(class = "metric-value", paste0(scales::number(pct, accuracy = 0.1, decimal.mark = ","), "%"))
  })
  ...
  output$mapa_m_t1 <- renderLeaflet({
    ...
    if (is.null(input$depto_t1) || input$depto_t1 == "Todos") {
      ...
      shp <- dptos_sf %>%
        dplyr::left_join(df_ag, by = "COD_DPTO2") %>%
        sf::st_as_sf()
      ...
      df_ag <- df_raw %>%
        dplyr::filter(COD_DPTO2 == cod_dep) %>%
        dplyr::group_by(COD_DPTO2, COD_MUN5) %>%
        dplyr::summarise(
          monto    = sum(VALOR_CREDITO_REAL, na.rm = TRUE),
          creditos = sum(NUMERO_CREDITO, na.rm = TRUE),
          .groups  = "drop"
      ...
      shp <- mpios_sf %>%
        dplyr::filter(COD_DPTO2 == cod_dep) %>%
        dplyr::left_join(df_ag, by = c("COD_DPTO2", "COD_MUN5")) %>%
        dplyr::mutate(
    ...
    if (ind == "monto") {
      ...
      legend_lab <- leaflet::labelFormat(
      ...
      legend_lab   <- leaflet::labelFormat(big.mark = ".", digits = 0)
    }
    
    m <- leaflet::leaflet(options = leaflet::leafletOptions(zoomSnap = 0.25)) %>%
      leaflet::addProviderTiles(leaflet::providers$CartoDB.Positron)
  ...
  build_map_t1_export <- reactive({
    ...
    zoom <- if (tipo_mapa == "depto") 8 else 10
    
    legend_title <- if (ind == "monto") "Monto total del crédito (real)" else "Número de créditos"
    ...
    m %>%
      leaflet::addLegend("bottomright", pal = pal, values = shp$valor, title = legend_title)
  })
  ...
  build_top5_t1 <- reactive({
    ...
    if (is.null(input$depto_t1) || input$depto_t1 == "Todos") {
      df <- df_raw %>%
        ...
        dplyr::left_join(
          dptos_sf %>% sf::st_drop_geometry() %>% dplyr::select(COD_DPTO2, NOM_DPTO),
          by = "COD_DPTO2"
        ) %>%
        dplyr::mutate(nombre = title_case_es(NOM_DPTO))
    } else {
      cod_dep <- dptos_sf$COD_DPTO2[dptos_sf$NOM_DPTO == input$depto_t1][1]
      ...
      df <- df_raw %>%
        ...
        dplyr::left_join(
          mpios_sf %>% sf::st_drop_geometry() %>% dplyr::select(COD_DPTO2, COD_MUN5, NOM_MPIO),
          by = c("COD_DPTO2", "COD_MUN5")
        ) %>%
        dplyr::mutate(nombre = title_case_es(NOM_MPIO))
    }
    
    if (nrow(df) == 0) return(NULL)
    
    if (ind == "monto") {
      df      <- df %>% dplyr::arrange(dplyr::desc(monto)) %>% dplyr::slice_head(n = 10)
      y_vals  <- df$monto / 1e9
      labels  <- scales::number(y_vals, big.mark = ".", decimal.mark = ",", accuracy = 0.1)
      y_title <- "Monto total (miles de millones)"
    } else {
      df      <- df %>% dplyr::arrange(dplyr::desc(creditos)) %>% dplyr::slice_head(n = 10)
      y_vals  <- df$creditos
      labels  <- scales::number(y_vals, big.mark = ".", decimal.mark = ",", accuracy = 1)
      y_title <- "Número de créditos"
    }
    ...
    g <- ggplot2::ggplot(df, ggplot2::aes(x = reorder(nombre, y_val), y = y_val)) +
      ggplot2::geom_col(fill = "#8d6e63") +
      ggplot2::geom_text(ggplot2::aes(y = y_val / 2, label = label_val), color = "white", size = 3.7) +
      ggplot2::coord_flip() +
      ggplot2::labs(title = "", x = NULL, y = y_title) +
  ...
  base_filtrada <- reactive({
    df <- finagro_fast
    if (!is.null(input$ano)) df <- df %>% dplyr::filter(ano == as.numeric(input$ano))
    if (!is.null(input$depto_t2) && input$depto_t2 != "Todos") df <- df %>% dplyr::filter(NOM_DPTO == input$depto_t2)
    if (!is.null(input$mpio_t2)  && input$mpio_t2  != "Todos") df <- df %>% dplyr::filter(NOM_MPIO == input$mpio_t2)
    if (!is.null(input$eslabon_t2) && input$eslabon_t2 != "Todos") df <- df %>% dplyr::filter(ESLABON_TC == input$eslabon_t2)
  ...
  build_serie_tiempo <- reactive({
    df <- base_filtrada() %>%
      ...
      dplyr::mutate(mes_lbl = factor(mes, levels = 1:12, labels = mes_labels))
    ...
    plotly::plot_ly(df, x = ~mes_lbl) %>%
      ...
      plotly::layout(
        ...
        yaxis2 = list(
          title = "Miles de millones (reales)", overlaying = "y", side = "right",
  ...
  build_top_lineas_eslabon <- reactive({
    df <- base_filtrada() %>%
      dplyr::group_by(LINEA_CREDITO, ESLABON_TC) %>%
      dplyr::summarise(monto = sum(VALOR_CREDITO_REAL, na.rm = TRUE), .groups = "drop")
    
    top_lines <- df %>%
      dplyr::group_by(LINEA_CREDITO) %>%
      dplyr::summarise(monto_total = sum(monto, na.rm = TRUE), .groups = "drop") %>%
      dplyr::arrange(dplyr::desc(monto_total)) %>%
      dplyr::slice_head(n = 5) %>%
      dplyr::pull(LINEA_CREDITO)
    
    df_top <- df %>%
      dplyr::filter(LINEA_CREDITO %in% top_lines) %>%
      dplyr::mutate(
    ...
    linea_order <- df_top %>%
      dplyr::group_by(LINEA_TC) %>%
      dplyr::summarise(monto_total = sum(monto_m, na.rm = TRUE), .groups = "drop") %>%
      dplyr::arrange(monto_total) %>%
      dplyr::pull(LINEA_TC)
    
    df_top <- df_top %>%
      dplyr::mutate(LINEA_TC = factor(LINEA_TC, levels = linea_order))
  ...
  output$top_lineas_eslabon <- renderPlotly({
    build_top_lineas_eslabon()
  })
  
  build_sankey <- reactive({
    df_links <- base_filtrada() %>%
      dplyr::group_by(LINEA_CREDITO, ESLABON_TC) %>%
      dplyr::summarise(value = sum(VALOR_CREDITO_REAL, na.rm = TRUE) / 1e9, .groups = "drop") %>%
      dplyr::mutate(
        LINEA_TC  = title_case_es(LINEA_CREDITO),
        ESLABON_F = factor(ESLABON_TC, levels = ESLABON_LEVELS)
      )
  ...
  tabla_export <- reactive({
    dplyr::bind_rows(
      base_t1() %>% dplyr::mutate(pestana = "Tab 1"),
      base_filtrada() %>% dplyr::mutate(pestana = "Tab 2")
    ) %>%
      dplyr::transmute(
        pestana,
        ano,
