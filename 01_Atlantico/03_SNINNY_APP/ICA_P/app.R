...
# ---- Utilidad: Title Case en español (robusta a encoding) ----
...
# =========================================================
# EXPORT (estilo IDM) — RMD FIJO EN RAÍZ
# =========================================================
...
pal4_vec_ica <- grDevices::colorRampPalette(c("#f0f8f4","#9fd9c0","#2ea56f","#0f7e4f"))(4)
  ...
  tags$head(
    tags$style(HTML("
      ...
      @media (max-width: 768px){
        .content-grid{ grid-template-columns:1fr; grid-template-rows:auto auto auto; }
        .viz-map{ grid-row:auto; }
      }
  ...
  div(
    ...
    div(
      ...
      div(
        ...
        div(class="filter", div(class="filter-label","¿En qué departamento?"), uiOutput("depto_ui")),
        div(class="filter", div(class="filter-label","¿Algún municipio en particular?"), uiOutput("muni_ui")),
        div(class="filter",
    ...
    div(
      ...
      div(
        ...
        div(class="viz-body", leafletOutput("mapa", height="100%")),
        div(class="map-note",
      ...
      div(
        class="card viz-card",
        div(class="card-title d-flex justify-content-between align-items-center",
            span(uiOutput("titulo_serie")),
            downloadButton("dl_png_serie","Descargar PNG", class="btn-unified")
        ),
        div(class="viz-body", plotlyOutput("serie", height="100%"))
      ),
      
      div(
        class="card viz-card",
        div(class="card-title d-flex justify-content-between align-items-center",
            span(uiOutput("titulo_top")),
            downloadButton("dl_png_top","Descargar PNG", class="btn-unified")
        ),
        div(class="viz-body", plotlyOutput("top10", height="100%"))
      )
    ),
...
server <- function(input, output, session){
  ...
  output$anio_ui <- renderUI({
    ...
    selectInput("anio", NULL, choices = yy, selected = max(yy))
  })
  ...
  output$muni_ui <- renderUI({
    ...
    selectizeInput(
      "muni", NULL,
      choices  = c("Todos"="Todos", stats::setNames(muni$CODMUN, muni$MUNICIPIO_D)),
      selected = "Todos",
      options  = list(placeholder = "Buscar municipio...", maxOptions = 5000)
    )
  })
  ...
  output$titulo_serie <- renderUI({
    htmltools::HTML("<strong>¿Cómo ha evolucionado el inventario de animales a lo largo del tiempo?</strong>")
  })
  ...
  observeEvent(input$mapa_shape_click, {
    ...
    if (en_deptos) {
      ...
      if (cl$id %in% (mpios_sf |> sf::st_drop_geometry() |> dplyr::pull(CODMUN))) {
        updateSelectInput(session, "muni", selected = cl$id)
      }
    }
  })
  ...
  build_serie_plotly <- function(){
    ...
    tickvals <- pretty(range(ts$valor, na.rm=TRUE), n=6)
    ticktext <- vapply(tickvals, fmt_km, character(1))
    
    plot_ly(
      ts, x=~year, y=~valor, type="scatter", mode="lines+markers",
  ...
  build_top_plotly <- function(){
    df <- datos_base() |>
      ...
      dplyr::mutate(MUNICIPIO_D = safe_utf8(MUNICIPIO_D))
  ...
  map_widget_export <- reactive({
    ...
    base_map <- leaflet::leaflet(options = leaflet::leafletOptions(minZoom = 12, maxZoom = 12, zoomSnap = 0.25)) %>%
  ...
  output$dl_png_mapa <- downloadHandler(
    filename = function(){
      ...
      paste0("ICA_mapa_", input$especie %||% "NA", "_", dep_tag, "_", mun_tag, "_", input$anio %||% "NA", "_", Sys.Date(), ".png")
    },
  ...
  output$dl_png_serie <- downloadHandler(
    filename = function(){
      paste0("ICA_serie_", input$especie %||% "NA", "_", Sys.Date(), ".png")
    },
    content  = function(file){
  ...
  output$dl_png_top <- downloadHandler(
    filename = function(){
      paste0("ICA_top10_", input$especie %||% "NA", "_", input$anio %||% "NA", "_", Sys.Date(), ".png")
    },
    content  = function(file){
  ...
  output$dl_csv_expl <- downloadHandler(
    filename = function(){
      dep_tag <- if (is.null(input$depto) || input$depto=="Todos") "Colombia" else input$depto
      mun_tag <- if (is.null(input$muni)  || input$muni =="Todos") "Todos"   else input$muni
      paste0("ICA_base_filtrada_", input$especie %||% "NA", "_", dep_tag, "_", mun_tag, "_", input$anio %||% "NA", "_", Sys.Date(), ".csv")
    },
    content = function(file){
  ...
  output$dl_reporte_pdf <- downloadHandler(
    filename = function(){
      dep_tag <- if (is.null(input$depto) || input$depto=="Todos") "Colombia" else input$depto
      mun_tag <- if (is.null(input$muni)  || input$muni =="Todos") "Todos"   else input$muni
      paste0("Informe_descargable_ICA_", input$especie %||% "NA", "_", dep_tag, "_", mun_tag, "_", input$anio %||% "NA", "_", Sys.Date(), ".pdf")
    },
    content = function(file){
