...
# ------------------------------
# 0) Parámetros del dpto foco
# ------------------------------
DPTO_FOCO_NOMBRE <- "Santander"
DPTO_FOCO_COD    <- "68"   # DANE dpto Santander

TAB1_TITLE <- paste0("Remisión de alimentos desde ", DPTO_FOCO_NOMBRE)
TAB2_TITLE <- paste0("Recepción de alimentos en ", DPTO_FOCO_NOMBRE)
...
ui <- fluidPage(
  ...
  div(
    ...
    tabsetPanel(
      ...
      # =====================================================
      # TAB 1 — REMISIÓN DESDE SANTANDER (destinos)
      # =====================================================
      tabPanel(
        TAB1_TITLE, br(),
        ...
        div(
          ...
          div(
            ...
            div(
              ...
              div(class="card-title", strong("¿En qué departamentos se concentra la mayor cantidad de alimentos remitidos desde el departamento priorizado?")),
        ...
        div(
          ...
          div(class="header-row",
              div(class="card-title", strong("¿Cuál es la composición de alimentos remitidos desde el departamento?")),
      ...
      # =====================================================
      # TAB 2 — RECEPCIÓN EN SANTANDER (orígenes)
      # =====================================================
      tabPanel(
        TAB2_TITLE, br(),
        ...
        div(
          ...
          div(
            ...
            div(
              class = "card viz-card viz-map",
              div(class="card-title", strong("¿En qué departamentos se concentra la mayor cantidad de alimentos recibidos en el departamento priorizado?")),
        ...
        div(
          class="card",
          div(class="header-row",
              div(class="card-title", strong("¿Cuál es la composición de alimentos recibidos en el departamento?")),
              div(style="font-size:12px;color:#6b7280;",
...
server <- function(input, output, session){
  ...
  # ---------------------------------
  # Base foco para choices (sin circularidad)
  # ---------------------------------
  base_from_foco <- reactive({
    df <- base_sipsa %>% filter(cod_dpto_o == DPTO_FOCO_COD)  # ✅ salidas DESDE Santander
  ...
  base_to_foco <- reactive({
    df <- base_sipsa %>% filter(cod_dpto_d == DPTO_FOCO_COD)  # ✅ entradas HACIA Santander
  ...
  output$territorio_ui_t1 <- renderUI({
    opts <- sort(unique(na.omit(base_from_foco()$departamento_d)))
    pickerInput(
      "territorio_t1", NULL,
  ...
  output$territorio_ui_t2 <- renderUI({
    opts <- sort(unique(na.omit(base_to_foco()$departamento_o)))
    pickerInput(
      "territorio_t2", NULL,
      choices  = c("Todos", opts),
      selected = "Todos",
  ...
  # =========================================================
  # TAB 1 — Contexto (desde Santander -> destinos)
  # =========================================================
  ctx_t1 <- reactive({
  ...
  # =========================================================
  # TAB 2 — Contexto (hacia Santander <- orígenes)
  # =========================================================
  ctx_t2 <- reactive({
  ...
  # =========================================================
  # TAB 1 — MAPA (destinos, desde Santander)
  # =========================================================
  ...
  badge_t1 <- reactive({
    ...
    htmltools::HTML(sprintf(
      '<div style="background:#fff;padding:6px 10px;border-radius:8px;
       ...
       </div>', htmltools::htmlEscape(DPTO_FOCO_NOMBRE), yr, ms, htmltools::htmlEscape(tr)
    ))
  })
  ...
  observeEvent(input$tabs_sipsa, {
    if (!is.null(input$tabs_sipsa) && input$tabs_sipsa == TAB1_TITLE) {
  ...
  # =========================================================
  # TAB 2 — MAPA (orígenes, hacia Santander)
  # =========================================================
  ...
  observeEvent(input$tabs_sipsa, {
    if (!is.null(input$tabs_sipsa) && input$tabs_sipsa == TAB2_TITLE) {
  ...
  output$dl_png_map_t1 <- downloadHandler(
    filename = function() paste0("SIPSA_mapa_tab1_destino_", DPTO_FOCO_NOMBRE, "_", Sys.Date(), ".png"),
    content  = function(file){
      shp <- mapa_t1_sf()
      widget <- if (is.null(shp)) {
        leaflet() %>% addProviderTiles(providers$CartoDB.Positron) %>% setView(MAP_LNG, MAP_LAT, MAP_ZOOM)
      } else {
        pal <- palBin4(shp$ton)
  ...
  output$dl_png_map_t2 <- downloadHandler(
    filename = function() paste0("SIPSA_mapa_tab2_origen_", DPTO_FOCO_NOMBRE, "_", Sys.Date(), ".png"),
    content  = function(file){
      shp <- mapa_t2_sf()
      widget <- if (is.null(shp)) {
        leaflet() %>% addProviderTiles(providers$CartoDB.Positron) %>% setView(MAP_LNG, MAP_LAT, MAP_ZOOM)
