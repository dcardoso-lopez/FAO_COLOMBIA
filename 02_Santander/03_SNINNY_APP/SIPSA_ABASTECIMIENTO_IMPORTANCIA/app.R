...
# ✅ CAMBIO: cada pestaña contiene TODOS sus filtros + sus visualizaciones
# (FOCO = SANTANDER / DPTO = 68)
# =========================================================

DPTO_FOCO_NOMBRE    <- "Santander"
DPTO_FOCO_COD       <- "68"
CAPITAL_FOCO_NOMBRE <- "Bucaramanga"
...
rds_candidates <- c(
  ...
  file.path(data_dir, "base_importancia_santander.rds")
)
...
# =========================================================
# Construcción base_sipsa (robusta)
#   Caso A: ya trae kg_total_origen + kg_a_santander
#   Caso B: base larga con destino cod_dpto_d + kg (se calcula hacia FOCO)
# =========================================================
...
kg_tot_col <- pick_first(
  nms,
  c("kg_total_origen","kg_total","kg_origen_total","kg_total_envia","kg_total_salida")
)

foco_slug <- slug_es(DPTO_FOCO_NOMBRE)  # "santander"
kg_foco_candidates <- unique(c(
  paste0("kg_a_", foco_slug),
  paste0("kg_", foco_slug),
  paste0("kg_hacia_", foco_slug),
  paste0("kg_to_", foco_slug),
  "kg_a_santander","kg_santander","kg_hacia_santander","kg_to_santander"
))
kg_foco_col <- pick_first(nms, kg_foco_candidates)

cod_d_col    <- pick_first(nms, c("cod_dane_dpto_d","dane_cod_dpto_d","cod_dpto_d","cod_depto_d"))
kg_long_col  <- pick_first(nms, c("cantkg_total","cant_kg_total","kg","kilogramos","cantidad_kg","cantidad"))

pct_col <- pick_first(nms, c("pct_importancia","porc_importancia","pct","porcentaje"))

build_from_preagg <- !is.na(kg_tot_col) && !is.na(kg_foco_col)
build_from_long   <- is.na(kg_foco_col) && !is.na(cod_d_col) && !is.na(kg_long_col)

if (!build_from_preagg && !build_from_long) {
  stop(
    "Tu .rds no tiene cómo construir 'kg hacia Santander'.\n",
    "- Intenté columna precomputada: ", paste(kg_foco_candidates, collapse = ", "), "\n",
    "- Y también intenté base larga con destino ('cod_*_d') + kg ('cantkg_total'/kg).\n\n",
    "Solución:\n",
    "1) Genera un .rds para Santander que incluya kg_a_santander + kg_total_origen, o\n",
    "2) Usa una base larga que tenga COD_DANE_DPTO_O, COD_DANE_DPTO_D y CantKg_total."
  )
}

if (build_from_preagg) {
  base_sipsa <- df0 %>%
    dplyr::transmute(
      ...
      kg_a_foco       = parse_num_co(.data[[kg_foco_col]]),
    ...
    dplyr::mutate(
      kg_a_foco = dplyr::coalesce(kg_a_foco, 0),
      pct = ifelse(kg_total_origen > 0, (kg_a_foco / kg_total_origen) * 100, NA_real_),
      ton_den = kg_total_origen / 1000,
      ton_num = kg_a_foco       / 1000
    ) %>%
  ...
  base_sipsa <- df0 %>%
    dplyr::transmute(
      ...
      grupo = title_case_es(.data[[gcol]]),
      cod_dpto_o = pad_dpto(.data[[cod_o_col]]),
      cod_dpto_d = pad_dpto(.data[[cod_d_col]]),
      departamento_o = if (!is.na(dep_o_col)) title_case_es(.data[[dep_o_col]]) else NA_character_,
      kg = parse_num_co(.data[[kg_long_col]])
    ) %>%
    dplyr::filter(is.finite(anio), anio >= 2018) %>%
    dplyr::filter(!is.na(cod_dpto_o), cod_dpto_o != "") %>%
    ...
    dplyr::summarise(
      departamento_o = dplyr::first(na.omit(departamento_o)) %||% NA_character_,
      ton_den = sum(kg, na.rm = TRUE) / 1000,
      ton_num = sum(kg[cod_dpto_d == DPTO_FOCO_COD], na.rm = TRUE) / 1000,
      pct = ifelse(sum(kg, na.rm = TRUE) > 0,
                   (sum(kg[cod_dpto_d == DPTO_FOCO_COD], na.rm = TRUE) / sum(kg, na.rm = TRUE)) * 100,
                   NA_real_),
      .groups = "drop"
    )
}
...
prefer_name  <- paste0("rutas_ORS_", tolower(DPTO_FOCO_NOMBRE), "_capitales_con_contexto.gpkg")
...
read_gpkg_layers <- function(gpkg_path){
  ...
  pick_route_layer <- function(lyr){
    cand <- lyr[grepl("rutas", lyr, ignore.case = TRUE) & grepl("capital", lyr, ignore.case = TRUE)]
    if (length(cand)) return(cand[1])
    cand2 <- lyr[grepl("rutas", lyr, ignore.case = TRUE)]
    if (length(cand2)) return(cand2[1])
    NA_character_
  }
  
  get_layer <- function(name){
    if (is.na(name) || !(name %in% lyr)) return(NULL)
    x <- sf::st_read(gpkg_path, layer = name, quiet = TRUE)
    janitor::clean_names(x)
  }
  
  rutas_layer <- pick_route_layer(lyr)
  rutas <- get_layer(rutas_layer)
...
# =========================================================
# UI helper: caja de filtros por pestaña
# =========================================================
  ...
  div(
    ...
    div(
      ...
      tabsetPanel(
        ...
        tabPanel(
          paste0("¿Cuánto de lo que envía cada departamento llega a ", DPTO_FOCO_NOMBRE, "?"),
          
          # ✅ filtros dentro del TAB 1
          filters_box("t1"),
        ...
        tabPanel(
          paste0("¿Cuáles son corredores donde se mueve el abastecimiento hacia ", DPTO_FOCO_NOMBRE, "?"),
          
          # ✅ filtros dentro del TAB 2
          filters_box("t2"),
          
          div(
            class="card",
            div(class="card-title",
                strong(paste0("¿Cuáles son los corredores de volúmenes de envío de alimentos al departamento ", DPTO_FOCO_NOMBRE, "?"))),
...
server <- function(input, output, session){
  ...
  # =========================================================
  # FILTROS POR PESTAÑA (IDs únicos)
  # =========================================================
  ...
  output$dptos_ui_t1 <- renderUI({
    ...
    choices <- c("Todos" = "Todos", stats::setNames(dpts$cod_dpto, dpts$dpto_nm))
    pickerInput(
      "dptos_t1", NULL,
      choices  = choices,
      selected = "Todos",
  ...
  mapa_imp_sf_t1 <- reactive({
    ...
    # no colorear el dpto foco como "origen"
  ...
  draw_map_imp <- function(){
    mdat <- tryCatch(mapa_imp_sf_t1(), error = function(e) NULL)
  ...
  observeEvent(list(input$anio_t1, input$grupos_t1, input$dptos_t1), {
    draw_map_imp()
  }, ignoreInit = FALSE)
  ...
  ton_by_dpto_t2 <- reactive({
    df <- datos_filtrados_t2() %>%
      dplyr::group_by(cod_dpto_o) %>%
      dplyr::summarise(
        ton_to_foco = sum(ton_num, na.rm = TRUE),
        ton_total   = sum(ton_den, na.rm = TRUE),
        .groups = "drop"
      ) %>%
      dplyr::mutate(cod_dpto_o = pad_dpto(cod_dpto_o))
    
    df$ton_to_foco[df$cod_dpto_o == DPTO_FOCO_COD] <- NA_real_
  ...
  rutas_con_ton <- reactive({
    ...
    validate(need(!is.null(rts) && inherits(rts,"sf") && nrow(rts) > 0,
                  "No hay capa de rutas en el .gpkg o está vacía."))
    ...
    out <- rts %>%
      ...
      dplyr::mutate(
        ton_to_foco = as.numeric(ton_to_foco),
        ton_total   = as.numeric(ton_total),
        ton_route   = ifelse(is.na(ton_to_foco) | ton_to_foco < 0, 0, ton_to_foco),
  ...
  draw_map_routes <- function(){
    ...
    # asegurar nombres bonitos desde dept_sf
    ...
    # filtro por dptos del TAB 2 (mantener foco)
    if (!sel_is_all(input$dptos_t2)) {
      sel_codes <- unique(as.character(input$dptos_t2))
      sel_codes <- sel_codes[sel_codes != "Todos"]
    ...
    # paleta verde-amarillo diferenciada por tramos
    ...
    proxy %>%
      leaflet::addPolylines(
        ...
        labelOptions = hover_label_opts,
        popup = ~paste0(
          "<strong>Ruta ", toupper(CAPITAL_FOCO_NOMBRE), " → capital (Departamento):</strong> ", dpto_show,
