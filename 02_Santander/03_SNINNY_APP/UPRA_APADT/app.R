# app_apadt.R — UPRA APADT (estilo FA)
# ✅ Botones en la MISMA POSICIÓN del código anterior:
#    - Mapa: botón PNG en el título (derecha)
#    - Top10: botón PNG en el título (derecha)
#    - Footer: CSV + PDF en barra inferior (derecha)
# ✅ Zoom consistente en:
#    - Visualización (leafletProxy)
#    - Descarga PNG (widget dedicado para webshot2)
# ✅ PDF: genera primero PNGs y luego renderiza Rmd
# ✅ FIX: usar SIEMPRE leaflet::addLegend (evita choque con quantmod::addLegend)
...
# Heurística simple para zoom “más cerca” (consistente vista/descarga)
...
server <- function(input, output, session){
  ...
  # ---------- Widget Leaflet para PNG (descarga) ----------
  # (IMPORTANTE: aquí replicamos el zoom “más cerca” que usamos en vista)
  map_widget <- reactive({
    d <- base_filtrada()
    ...
    if (is.null(d) || nrow(d) == 0) {
      return(base_map %>% leaflet::setView(-74.3, 5, 5.8))
    }
    ...
    if (is.null(input$f_dep) || input$f_dep == "Todos") {
      ...
      base_map %>%
        ...
        leaflet::addPolygons(
          ...
          label = ~lapply(
            paste0("<b>", nombre, "</b><br>", label_txt_global, ": ",
      ...
      base_map %>%
        ...
        leaflet::setView(lng, lat, zoom = z) %>%   # <- “zoom extra” consistente
        leaflet::addPolygons(
          data = shp,
          layerId = ~COD_MUN5,
          fillColor = ~pal(valor),
          ...
          label = ~lapply(
            paste0("<b>", MUNICIPIO_MOSTRAR, " (", DEP_MOSTRAR, ")</b><br>",
                   label_txt_global, ": ",
                   ifelse(is.na(valor), "NA",
                          if (ind == "prop_apadt") fmt_pct(valor, acc = 0.1) else fmt_int(valor)),
  ...
  # ---------- Actualización interactiva del mapa (vista) ----------
  observe({
    d <- base_filtrada()
    if (is.null(d) || nrow(d) == 0) {
      leaflet::leafletProxy("map_apadt") %>% leaflet::clearShapes() %>% leaflet::clearControls()
    ...
    if (is.null(input$f_dep) || input$f_dep == "Todos") {
      ...
      leaflet::leafletProxy("map_apadt", data = shp) %>%
        ...
        leaflet::setView(-74.3, 5, 5.8) %>%  # <- zoom “más cerca” Colombia
        leaflet::addPolygons(
          layerId = ~COD_DPTO2,
          fillColor = ~pal(valor),
          color = COLOR_BORDE, weight = 0.7, fillOpacity = 0.9,
  ...
  output$dl_bar_png <- downloadHandler(
    ...
    content = function(file){
      ok <- save_widget_png(bar_plot_obj(), file, vwidth = PNG_VWIDTH, vheight = PNG_VHEIGHT, delay = 0.8)
      if (!ok) stop("No se pudo generar el PNG del top10. Revisa webshot2/Chromium.")
    }
  )
  
  # -------- Informe PDF (RMarkdown) --------
  output$dl_report_pdf <- downloadHandler(
    ...
    content = function(file) {
      ...
      # ✅ Generar PNGs con el MISMO zoom que la vista (map_widget)
