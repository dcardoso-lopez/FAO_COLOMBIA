# app_upra.R — UPRA (FA)
# ✅ FIX numéricos + filtro % SOLO cuando el indicador es proporción
# ✅ SIN “pantallazo” (sin html2canvas)
# ✅ MODS:
#   1) Más zoom (Colombia + depto con zoom extra)
#   2) Descargar mapa en .PNG (webshot2 desde el widget Leaflet)
#   3) La leyenda queda dentro del PNG (se captura el widget completo)
#   4) Descargar Top10 en PNG (webshot2)
#   5) Descargar CSV (base filtrada)
#   6) Descargar Informe PDF (RMarkdown) generando PNG antes
...
# ✅ RMD (acepta .Rmd o .rmd; puede estar en raíz o en data)
candidatos_rmd <- c(
...
ruta_rmd <- candidatos_rmd[file.exists(candidatos_rmd)][1]
if (is.na(ruta_rmd)) ruta_rmd <- ""
...
num_or_na <- function(x){
  ...
  # Si no hay coma -> decimal "." (miles "," si viniera)
...
build_interval_labels <- function(breaks, as_percent = FALSE){
  ...
  vapply(
    ...
    function(i) format_interval_label(breaks[i], breaks[i+1], as_percent = as_percent, is_first = (i==1)),
    character(1)
  )
}
...
# ---------- Carpeta de export (PNG y reporte) ----------
...
col_ano_u     <- pick_col(upra_raw, "ano", "^a(n|ñ)o$|year")
...
col_mun_cod_u <- pick_col(upra_raw, "COD_DANE_MUNIC_D", "MUNI.*COD|COD.*MUNI|DANE.*MUNI|COD_MUN5|MPIO")
col_mun_nom_u <- pick_col(upra_raw, "MUNICIPIO_D", "MUNICIP")
col_area_mpio <- pick_col(upra_raw, "area_mpio_ha", "area.*mpio.*ha|mpio.*ha|area.*municip")
...
ui <- fluidPage(
  ...
  tags$head(
    ...
    .filter-label{
      ...
      font-size:14px;font-weight:500;letter-spacing:.2px;color:#111827;margin-bottom:4px;
    }
...
server <- function(input, output, session){
  ...
  empty_plot <- function(texto = "Sin datos para los filtros seleccionados.") {
    plotly::plotly_empty(type = "scatter", mode = "markers") %>%
      plotly::layout(
        annotations = list(x=.5,y=.5,text=texto,showarrow=FALSE,
  ...
  # ---------- Plotly Top 10 (widget reutilizable para render y PNG) ----------
  bar_plot_obj <- reactive({
    ...
    if (ind == "prop_fa") {
      ...
      hover_tpl <- "%{y}<br>Proporción: %{x:.1%}<extra></extra>"
    } else {
      txt_vals  <- fmt_num(d1$val, digs = 1)
      x_title   <- "Hectáreas"
      hover_tpl <- "%{y}<br>Área apta acumulada: %{text} ha<extra></extra>"
    }
  ...
  # ---------- Widget Leaflet “completo” (incluye leyenda) ----------
  map_widget <- reactive({
    d <- base_filtrada()
    ind <- input$f_ind %||% "area_fa_ha"
    label_txt_global <- if (ind == "prop_fa") "Porcentaje" else "Área (ha)"
    
    # tiles con CORS para webshot
    tiles <- function(mp){
      mp %>% addProviderTiles(
    ...
    if (is.null(d) || nrow(d) == 0) {
      return(
        tiles(leaflet(options = leafletOptions(minZoom = 4, maxZoom = 12, zoomSnap = 0.25))) %>%
    ...
    if (map_mode() == "CO") {
      ...
      if (ind == "prop_fa") {
        ...
        mp <- mp %>% addLegend(
          ...
          pal = pal, values = shp$valor, title = "Hectáreas",
  ...
  output$dl_report_pdf <- downloadHandler(
    ...
    content = function(file) {
      ...
      if (!nzchar(ruta_rmd) || !file.exists(ruta_rmd)) {
        stop("No encuentro Informe_descargable.Rmd/.rmd en la raíz del proyecto ni en data/.")
      }
