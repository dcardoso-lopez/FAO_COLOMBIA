# app_idm_santander.R — Terridata IDM Santander (PNG mapa+leyenda + Top10 + PromDepto + CSV + PDF)
suppressWarnings({
...
# *** Filtro fijo: solo Santander ***
idm_raw <- idm_raw %>% dplyr::filter(DEPARTAMENTO_D == "SANTANDER")
...
dep_lookup <- idm %>%
  dplyr::select(COD_DANE_DPTO, DEPARTAMENTO_TC) %>%
  ...
  tags$head(
    tags$style(HTML("
      ...
      .btn-unified{
        ...
        font-weight:700 !important;border-radius:12px !important;padding:6px 10px !important;
      }
      ...
      .info-quantiles-note{margin-top:8px;font-size:12px;color:#4b5563;}
    "))
  ),
...
server <- function(input, output, session){
  ...
  get_cod_from_dep_tc <- function(dep_tc){
    ...
    j <- which(dep_lookup$DEPARTAMENTO_TC == dep_tc)[1]
    ...
    if (is.null(input$f_dep) || input$f_dep == "Todos") {
      ...
      base_map %>%
        ...
        leaflet::addPolygons(
          ...
          fillColor = ~pal(valor_pct), color="#666", weight=0.7, fillOpacity=0.9
        ) %>%
        leaflet::addLegend(position="bottomright", colors=cols_legend, labels=labels_legend, opacity=0.9, title="Porcentaje")
      ...
      base_map %>%
        ...
        leaflet::addPolygons(
          ...
          fillColor = ~pal(valor_pct), color="#666", weight=0.4, fillOpacity=0.9
        ) %>%
        leaflet::addLegend(position="bottomright", colors=cols_legend, labels=labels_legend, opacity=0.9, title="Porcentaje")
    }
  })
  ...
  build_plot_top <- reactive({
    ...
    d1 <- d %>%
      ...
      dplyr::mutate(txt = fmt_pct100(val_pct, acc = 0.1), lbl = MUNICIPIO_TC)
    ...
    if (!is.null(axis_info)) {
      ...
      shapes_list <- list(list(type="line", xref="x", yref="paper",
    ...
    plot_ly(d1 %>% dplyr::arrange(val_pct),
      ...
      layout(xaxis=xaxis_opts, yaxis=list(title="", automargin=TRUE),
             margin=list(l=10,r=10,b=10,t=10), showlegend=FALSE, shapes=shapes_list)
  })
  ...
  build_plot_depto <- reactive({
    ...
    plot_ly(d2 %>% dplyr::arrange(val_pct),
      ...
      layout(xaxis=xaxis_opts, yaxis=list(title="", automargin=TRUE),
             margin=list(l=10,r=10,b=10,t=10), showlegend=FALSE)
  })
  ...
  output$dl_map_png <- downloadHandler(
    ...
    content = function(file){
      ...
      ok <- save_widget_png(map_widget(), file, delay = dly)
  ...
  output$dl_dep_png <- downloadHandler(
    ...
    content = function(file){
      ...
      if (!ok) stop("No se pudo generar el PNG del promedio por departamento.")
    }
  )
  ...
  output$dl_csv <- downloadHandler(
    ...
    content = function(file){
      d <- base_filtrada()
      d_out <- d %>% mutate(valor_pct = scale_to_pct(valor)) %>%
        select(ano, COD_DANE_DPTO, DEPARTAMENTO_TC, COD_DANE_MUNI, MUNICIPIO_TC, valor, valor_pct)
  ...
  output$dl_reporte_pdf <- downloadHandler(
    ...
    content = function(file){
      ...
      # PNGs para el informe
      dly_map <- if (!is.null(dep_now) && dep_now != "Todos") PNG_DELAY_MUN else PNG_DELAY_CO
      ok_map <- save_widget_png(map_widget(),       IMG_MAP, delay = dly_map)
      ...
      if (!ok_map) stop("No se pudo generar idm_mapa.png")
      if (!ok_top) stop("No se pudo generar idm_top10.png")
      if (!ok_dep) stop("No se pudo generar idm_prom_depto.png")
      
      # CSV filtrado para el informe
      d <- base_filtrada()
      d_out <- d %>% mutate(valor_pct = scale_to_pct(valor)) %>%
        select(ano, COD_DANE_DPTO, DEPARTAMENTO_TC, COD_DANE_MUNI, MUNICIPIO_TC, valor, valor_pct)
      ...
      # Logo a Descargas + reemplazo placeholder si existe
