...
APP_DIR  <- "C:/Users/Dell/Universidad de los andes/FAO-SAT - Documentos/General/06_Scripts/FAO_COLOMBIA/02_Santander/03_SNINNY_APP/HANSEN_COBERTURA_BOSQUE"
...
format_int_es <- function(x){
  format_num_es(x, digits = 0)
}
format_pct_es <- function(x, digits = 1){
  paste0(format_num_es(x, digits = digits), "%")
}

# Formato corto K / M (es-CO)
...
make_bins4 <- function(values, clamp01 = FALSE){
  ...
  if (!length(v)) return(seq(0, 4))  # dummy
  ...
  if (length(qs) < 5) {
    qs <- seq(min(qs), max(qs), length.out = 5)
  }
  qs
}
...
ui <- fluidPage(
  ...
  div(
    ...
    div(
      class = "data-note",
      HTML("Cobertura neta de bosque (ha y %) a nivel departamental y municipal, con base Hansen 2000–2022.")
    ),
      ...
      div(
        ...
        div(
          ...
          div(class="filter-label","¿En que departamento?"),
          {
            ...
            ## Default: Atlántico seleccionado
            idx_atl <- which(
...
server <- function(input, output, session){
  ...
  agg_depto <- reactive({
    ...
    if (input$f_metric == "ha") {
      df |>
        ...
        dplyr::summarise(
          valor = sum(cobertura_ha, na.rm = TRUE),
          .groups = "drop"
        )
    } else {
      df |>
  ...
  # -------- Mapa inicial: deptos o mpios según filtro --------
  output$map_eva <- leaflet::renderLeaflet({
    req(input$f_depto, input$f_metric, input$f_anio)
    
    # Sin departamento filtrado -> vista nacional por departamentos
    if (is.null(input$f_depto) || input$f_depto == "Todos") {
      nivel_mapa("depto")
      depto_sel(NULL)
      ...
      leaflet::leaflet(mdat) |>
        ...
        leaflet::addPolygons(
          ...
          label = ~sprintf(
            ...
            DEPTO_TC,
            if (input$f_metric=="ha")
              paste0(format_short(valor)," Ha")
            else
              format_pct_es(valor, digits = 2)
          ),
  ...
  # -------- Redibujar niveles desde otros eventos --------
  dibujar_deptos <- function(){
    ...
    ttl <- if (input$f_metric=="ha") "Área de bosque que queda (Ha)"
    else "Porcentaje de bosque que queda (%)"
  ...
  dibujar_mpios <- function(dep){
    ...
    if (!is.null(input$f_mpio) && input$f_mpio != "Todos") {
      ...
      mdat <- mdat |>
        dplyr::mutate(
          valor_plot = dplyr::if_else(MUNICIPIO_D == sel_key, valor, NA_real_)
        )
    } else {
      mdat <- mdat |>
      ...
      leaflet::addPolygons(
        ...
        label=~sprintf(
          ...
          ifelse(
            ...
            "Sin información",
            if (input$f_metric=="ha")
              paste0(format_short(valor_plot)," Ha")
            else
              format_pct_es(valor_plot, digits = 2)
          )
        ),
        labelOptions=hover_label_opts_small,
        highlightOptions=leaflet::highlightOptions(color="black", weight=2, bringToFront=TRUE)
  ...
  # Si cambias el depto desde el filtro, seguimos usando la lógica de niveles
  ...
  series_data <- reactive({
    ...
    if (input$f_metric == "ha") {
      base |>
        dplyr::group_by(anio) |>
        dplyr::summarise(
          valor_total = sum(cobertura_ha, na.rm = TRUE),
          .groups = "drop"
        ) |>
        dplyr::arrange(anio)
    } else {
  ...
  output$plot_arriba <- plotly::renderPlotly({
    ...
    if (input$f_metric=="ha") {
      ylab        <- "Hectáreas"
      y_breaks    <- pretty(df$valor_total, n = 5)
  ...
  ranking_data <- reactive({
    ...
    if (input$f_metric=="ha") {
      df |>
        ...
        dplyr::mutate(
          muni_tc  = title_case_es(MUNICIPIO_RAW),
          depto_tc = title_case_es(DEPARTAMENTO_RAW)
        )
    } else {
      df |>
  ...
  output$ranking_abajo <- plotly::renderPlotly({
    ...
    if (input$f_metric == "ha") {
      ...
      xaxis_opts <- list(
        title    = "Porcentaje",
        tickvals = breaks,
        ticktext = format_pct_es(breaks, digits = 0),
        showgrid = FALSE,
        range    = c(0, 100)
      )
  ...
  output$dl_csv_expl <- downloadHandler(
    filename = function() paste0(
      "HANSEN_cobertura_neta_",
      safe_chr(input$f_metric), "_",
      safe_chr(input$f_anio), "_",
      Sys.Date(), ".csv"
    ),
  ...
  output$dl_png_series <- downloadHandler(
    ...
    content  = function(file){
      ...
      g <- ggplot(df, aes(x=anio, y=valor_total)) +
        ...
        labs(x="Año", y=ylab,
             title=paste0("Evolución anual de ", ylab)) +
  ...
  map_widget_simple <- reactive({
    if (nivel_mapa()=="depto"){
      ...
      leaflet::leaflet(mdat, options=leaflet::leafletOptions(zoomControl=FALSE)) |>
        leaflet::addProviderTiles(leaflet::providers$CartoDB.Positron) |>
        leaflet::addPolygons(
          fillColor=~pal(valor_plot), weight=0.4, color="#666", fillOpacity=0.9
        ) |>
        leaflet::addControl(html = htmltools::HTML(
          sprintf("<div style='font-weight:600;font-size:14px;background:#fff;
                   padding:6px 8px;border-radius:8px;border:1px solid #e6e6e6'>
  ...
  output$dl_png_mapa <- downloadHandler(
    filename = function() paste0("HANSEN_cobertura_mapa_",
                                 safe_chr(input$f_metric), "_", Sys.Date(), ".png"),
    content  = function(file){
    ...
    content  = function(file){
      ...
      if (input$f_metric == "ha") {
        ...
        g <- ggplot(
          plot_df,
          aes(x = valor_total,
              y = reorder(muni_label, valor_total))
        ) +
          geom_col(fill = BAR_COLOR) +
          geom_text(
            aes(
              x     = valor_total / 2,
              label = format_short(valor_total)
            ),
            color = "white", size = 3
          ) +
          scale_x_continuous(
          ...
          labs(
            x = "Cobertura neta (Ha)",
            y = NULL,
            title = paste0("Top-10 municipios por área de bosque (Ha) — ", safe_chr(input$f_anio))
          )
      } else {
        breaks <- seq(0, 100, 20)
        ...
        g <- ggplot(
          ...
          geom_text(
            aes(
              x     = valor_total / 2,
              label = format_pct_es(valor_total, digits = 1)
            ),
            color = "white", size = 3
          ) +
          scale_x_continuous(
            labels = function(x) format_pct_es(x, digits = 0),
            breaks = breaks,
            limits = c(0, 100),
            expand = expansion(mult = c(0, 0.05))
          ) +
          labs(
            x = "Porcentaje",
            y = NULL,
            title = paste0("Top-10 municipios por porcentaje de bosque (%) — ", safe_chr(input$f_anio))
          )
      }
