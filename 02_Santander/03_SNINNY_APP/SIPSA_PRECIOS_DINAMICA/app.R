...
MUNI_FOCO_COD       <- "68001"  # fallback si hay código municipal
...
# =========================================================
# Finder genérico de RDS
# =========================================================
find_prices_rds <- function(paths){
  for (p in paths) {
    if (!is.na(p) && file.exists(p) && !dir.exists(p) && grepl("\\.rds$", tolower(p))) return(p)
  }
  for (p in paths) {
...
# =========================================================
user_dir_or_prefix_1 <- "042_DANE_SIPSA-Precios"

candidate_paths_1 <- c(
  file.path(data_dir, "042_DANE_SIPSA-Precios.rds"),
  file.path(data_dir, "042_DANE_SIPSA-Precios.RDS"),
  paste0(user_dir_or_prefix_1, ".rds"),
  paste0(user_dir_or_prefix_1, ".RDS"),
  user_dir_or_prefix_1
)

rds_path_1 <- find_prices_rds(candidate_paths_1)
if (is.na(rds_path_1)) {
  stop(
    "No encontré el archivo de precios TAB 1 (.rds).\n\n",
    "Busqué en:\n- ", paste(candidate_paths_1, collapse = "\n- "), "\n\n",
    "Solución rápida:\n",
...
# Filtrar Santander si hay cod depto destino (sin impedir NA)
...
# =========================================================
user_dir_or_prefix_2 <- "042_DANE_SIPSA-Precios_indice2"

candidate_paths_2 <- c(
  file.path(data_dir, "042_DANE_SIPSA-Precios_indice2.rds"),
  file.path(data_dir, "042_DANE_SIPSA-Precios_indice2.RDS"),
  paste0(user_dir_or_prefix_2, ".rds"),
  paste0(user_dir_or_prefix_2, ".RDS"),
  user_dir_or_prefix_2
)

rds_path_2 <- find_prices_rds(candidate_paths_2)
if (is.na(rds_path_2)) {
  stop(
    "No encontré el archivo de precios TAB 2 (.rds).\n\n",
    "Busqué en:\n- ", paste(candidate_paths_2, collapse = "\n- "), "\n\n",
    "Solución rápida:\n",
    "1) Copia el .rds a: ", data_dir, "\n",
...
# =========================================================
user_dir_or_prefix_3 <- "042_DANE_SIPSA-Precios_indice3"

candidate_paths_3 <- c(
  file.path(data_dir, "042_DANE_SIPSA-Precios_indice3.rds"),
  file.path(data_dir, "042_DANE_SIPSA-Precios_indice3.RDS"),
  paste0(user_dir_or_prefix_3, ".rds"),
  paste0(user_dir_or_prefix_3, ".RDS"),
  user_dir_or_prefix_3
)

rds_path_3 <- find_prices_rds(candidate_paths_3)
if (is.na(rds_path_3)) {
  stop(
    "No encontré el archivo de precios TAB 3 (.rds).\n\n",
    "Busqué en:\n- ", paste(candidate_paths_3, collapse = "\n- "), "\n\n",
    "Solución rápida:\n",
    "1) Copia el .rds a: ", data_dir, "\n",
...
# Ciudad / municipio (código + nombre)
...
# Dpto opcional (para enriquecer etiquetas)
...
# Plaza / central / fuente (para fijar referencia de Bucaramanga SOLO centrales mayoristas)
...
# etiqueta ciudad (para plot / tabla)
prices3 <- prices3 %>%
  dplyr::mutate(
...
ui <- fluidPage(
  ...
  tags$head(
    tags$style(HTML("
      :root{ --accent-border:#ffe082; --plot-big-h: 600px; --plot-small-h: 260px; }
    ...
    div(
      ...
      tabsetPanel(
        ...
        tabPanel(
          ...
          div(
            class="card",
            div(class="card-title", strong(paste0("Diferenciales de precios por departamentos vs el departamento priorizado"))),
        ...
        # =========================================================
        # TAB 3 — Diferencias ENTRE CIUDADES (referencia: BGA centrales mayoristas)
        # =========================================================
        tabPanel(
          ...
          div(
            class="card",
            div(class="card-title", strong(paste0("Diferenciales de precios por ciudad vs central mayorista principal"))),
...
server <- function(input, output, session){
  ...
  # ======================
  # TAB 1 — filtros (SIN plaza/fuente)
  # ======================
  ...
  # ======================
  # TAB 2 — filtros (SIN plaza/fuente, SIN mes)
  # ======================
  ...
  # ======================
  # TAB 2 — Indicador por DEPARTAMENTO (sin mes)
  # comp = (promedio anual del dpto) - (promedio anual del dpto foco)
  # ======================
  comp_data <- reactive({
  ...
  # ======================
  # TAB 3 — Indicador por CIUDAD (sin mes)
  # comp = (promedio anual ciudad) - (promedio anual referencia: BGA centrales mayoristas)
  # La referencia usa SOLO registros de Bucaramanga y filtra plazas/tipos que parezcan "central mayorista"
  # ======================
  comp_data3 <- reactive({
    df <- datos_filtrados_t3()
    
    # 1) Construir promedio anual por ciudad
    m_city <- df %>%
    ...
    # 2) Referencia: Bucaramanga SOLO centrales mayoristas
    bga <- df %>%
      dplyr::filter(
    ...
    # Filtro "centrales mayoristas" (si hay columnas; si no, usa todo Bucaramanga)
    bga_ref <- bga
    ...
    validate(need(nrow(bga_ref) > 0, paste0(
      "Encontré Bucaramanga, pero no pude aislar 'centrales mayoristas' (revisa columnas tipo_plaza/plaza). ",
      "Como alternativa, ajusta el patrón en el código si el nombre viene distinto."
    )))
    
    # promedio anual de la referencia
    bga_ref_a <- bga_ref %>%
      dplyr::group_by(anio) %>%
      dplyr::summarise(precio_a = mean(precio_kg, na.rm = TRUE), .groups = "drop")
    
    foco_mean <- mean(bga_ref_a$precio_a, na.rm = TRUE)
    validate(need(is.finite(foco_mean), "No se pudo calcular el promedio de referencia (BGA centrales mayoristas)."))
  ...
  output$comp_plot3 <- renderPlotly({
    ...
    # Etiquetas: si hay demasiadas ciudades, etiqueta solo las 60 con mayor |comp|
    df_lab <- df %>%
