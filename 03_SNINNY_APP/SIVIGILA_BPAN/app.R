...
# - Tab 3: Storytelling de supervivencia y fallecimientos (títulos narrativos)
...
ui <- fluidPage(
  ...
  tags$head(
    tags$style(HTML("
      ...
      .filters{
        background:#fff;
        border:1px solid var(--borde-acento) !important;
        border-radius:16px;
        padding:14px 16px;
        margin-bottom:16px;
      ...
      /* Inputs (solo borde) */
      .selectize-input,.form-control{min-height:42px;border-radius:10px;border-color:var(--borde-acento) !important; box-shadow:none !important;}
      .selectize-input{padding:10px 12px}
      .selectize-input.focus, .form-control:focus{border-color:var(--borde-acento) !important; box-shadow:none !important;}
      .selectize-dropdown{border-color:var(--borde-acento) !important;}
      ...
      .info-badge{
        display:inline-flex;justify-content:center;align-items:center;width:18px;height:18px;margin-left:6px;border-radius:50%;
        font-size:12px;line-height:1;font-weight:700;background:#e5e7eb;color:#111827;cursor:help;border:1px solid #d1d5db;
      }
...
Úsalo para ver si tu territorio depende de otros para atender los casos o si actúa como centro receptor.">
        ...
        tabPanel(
          ...
          div(class="filters",
              div(class="filters-grid",
                  div(class="filter", div(class="filter-label","Sel. Año"), uiOutput("anio_b2_ui")),
                  div(class="filter", div(class="filter-label","Sel. Departamento:"),
                      selectInput("f_depto_b2", NULL, choices = c("Todos", dpt_lookup_bpan$DEP_D), selected = "Todos")),
                  div(class="filter", div(class="filter-label","Sel. Municipio:"),
          ...
          fluidRow(
            column(6,
                   div(class="card",
                       div(class="card-title", textOutput("map_bpan_incid_title")),
            ...
            column(6,
                   div(class="card",
                       div(class = "card-title",
                           "¿Qué departamentos tienen la situación más crítica de bajo peso al nacer? En términos de la incidencia por 100.000 habitantes"),
        ...
        # ---------------- TAB 3: Storytelling de fallecimientos/supervivencia ----------------
        tabPanel(
          title = "Fallecimientos y Supervivencia",
          br(),
          div(class="filters",
              div(class="filters-grid",
                  div(class="filter", div(class="filter-label","Sel. Año:"), uiOutput("anio_b3_ui")),
                  div(class="filter", div(class="filter-label","Sel. Departamento (ORIGEN):"), uiOutput("dep_o_b3_ui")),
                  div(class="filter", div(class="filter-label","Sel. Municipio (ORIGEN):"), uiOutput("mun_o_b3_ui")),
                  div(class="filter", div(class="filter-label","Sel. Departamento (DESTINO):"), uiOutput("dep_d_b3_ui")),
                  div(class="filter", div(class="filter-label","Sel. Municipio (DESTINO):"), uiOutput("mun_d_b3_ui"))
              )
          ),
          fluidRow(
            column(
              ...
              div(class="card",
                  div(class="card-title",
                      "Primero, la evolución: ¿cómo cambian en el tiempo los casos de supervivencia y fallecimientos?"),
                  plotlyOutput("serie_anual_tab3", height = 360)
              ),
              div(class="card",
                  div(class="card-title",
                      HTML('Luego, ¿en qué edades maternas se concentran los fallecimientos?
            ...
            column(
              ...
              div(class="card",
                  div(class="card-title",
                      HTML('Finalmente, ¿dónde es mayor la probabilidad de supervivencia? (Vivos/(Vivos+Muertes))
                           <span class="info-badge" data-bs-toggle="tooltip"
                                 title="Porcentaje de casos con resultado Vivo entre (Vivos+Muertes). Se calcula sobre registros con PAC_HOS válido (1 o 2).">i</span>')),
                  leafletOutput("map_superv_b3", height = 750)
              )
            )
          )
        )
...
server <- function(input, output, session){
  ...
  output$bar_bpan <- renderPlotly({
    ...
    plot_ly(
      data = df2,
      x = ~valor, y = ~factor(nombre, levels = df2$nombre),
  ...
  output$map_bpan_incid_title <- renderText({
    if (nivel_b2() == "mpios") {
      dep <- input$f_depto_b2 %||% ""
      paste0("¿En qué municipios es más probable que un recién nacido tenga bajo peso", 
             if (nzchar(dep)) paste0(" (", dep, ")?") else "?")
    } else {
      "¿En qué departamentos es más probable que un recién nacido tenga bajo peso?"
    }
  })
  ...
  observe({
    ...
    pal_cols_incid <- c("#fff9db", "#ffe082", "#ffd54f", "#fbc02d", "#c49000")
    if (nivel_b2()=="deptos") {
      dd <- agg_incid_depto()
      ...
      pal <- make_pal(shp$incid, pal_cols_incid)
      ...
      pal <- make_pal(shp$incid, pal_cols_incid); bb <- sf::st_bbox(shp)
      leaflet::leafletProxy("map_bpan_incid", data=shp) %>% leaflet::clearShapes() %>% leaflet::clearControls() %>%
  ...
  output$top_incid_dest_top20 <- renderPlotly({
    df <- base_b2() %>%
      ...
      mutate(incid = ifelse(POB > 0, (casos / POB) * 1e5, NA_real_),
    ...
    plot_ly(
      ...
      layout(
        title = list(text = "¿Qué departamentos tienen la situación más crítica de bajo peso al nacer? En términos de la incidencia por 100.000 habitantes", x = 0),
  ...
  output$edad_materna_barras <- renderPlotly({
    ...
    orden_niveles <- intersect(
      c("<10","10–14","15–19","20–24","25–29","30–34","35–39","40–44","45–49","50+","Sin dato"),
  ...
  output$mun_o_b3_ui <- renderUI({
    ...
    if (input$f_dep_o_b3 == "Todos") {
      ...
      mm <- bpan %>% dplyr::filter(DEP_O == input$f_dep_o_b3) %>% dplyr::distinct(MUN_O) %>% dplyr::arrange(MUN_O)
  ...
  output$mun_d_b3_ui <- renderUI({
    ...
    if (input$f_dep_d_b3 == "Todos") {
      ...
      mm <- bpan %>% dplyr::filter(DEP_D == input$f_dep_d_b3) %>% dplyr::distinct(MUN_D) %>% dplyr::arrange(MUN_D)
      selectInput("f_mun_d_b3", "Municipio (DESTINO)", choices = c("Todos", mm$MUN_D), selected = "Todos")
    }
  })
  base_b3 <- reactive({
    req(input$f_anio_b3)
  ...
  output$serie_anual_tab3 <- renderPlotly({
    ...
    if (!is.na(mes_col)) {
      df0 <- df0 %>% dplyr::mutate(MES = suppressWarnings(as.integer(.data[[mes_col]])),
                                   MES = dplyr::if_else(is.finite(MES) & MES >= 1 & MES <= 12, MES, 1L))
    } else df0 <- df0 %>% dplyr::mutate(MES = 1L)
    dfm <- df0 %>% dplyr::mutate(ANO = suppressWarnings(as.integer(ano)),
      ...
      dplyr::transmute(FECHA, vivos = cat_1, muertes = cat_2) %>% dplyr::arrange(FECHA)
    plot_ly(dfm, x = ~FECHA) %>%
      ...
      layout(xaxis = list(title = "Año–Mes", tickformat = "%Y-%m"),
  ...
  output$hist_edad_tab3 <- renderPlotly({
    df <- base_b3() %>%
      dplyr::filter(!is.na(edad), !is.na(PAC_HOS), PAC_HOS == 2L) %>%
      dplyr::mutate(
        edad = suppressWarnings(as.numeric(edad)),
    ...
    orden_niveles <- c("<10","10–14","15–19","20–24","25–29","30–34","35–39","40–44","45–49","50+","Sin dato")
    ...
    plot_ly(
      ...
      layout(xaxis = list(title = "Número de fallecimientos"),
             yaxis = list(title = "Edad de la madre (años)"),
             margin = list(l = 80, r = 10, b = 40, t = 10),
             showlegend = FALSE)
  })
  ...
  sel_cod_dep_b3 <- reactive({
    if (nivel_b3_map() == "mpios") {
      ...
      if (is.na(cod) || !nzchar(cod)) {
        cod <- base_b3() %>% dplyr::distinct(COD_DPTO2) %>% dplyr::pull(COD_DPTO2) %>% .[1]
      }
      cod
    } else NA_character_
  })
  agg_superv_depto_b3 <- reactive({
    base_b3() %>% dplyr::filter(!is.na(PAC_HOS), PAC_HOS %in% c(1L,2L)) %>%
  ...
  observe({
    # Paleta dorada solicitada
    gold_pal <- c("#c49000", "#fbc02d", "#ffd54f", "#ffe082", "#fff9db")
    ...
    if (nivel_b3_map() == "deptos") {
      ...
      pal  <- leaflet::colorBin(gold_pal, domain = shp$tasa, bins = bins, na.color = "#f0f0f0")
