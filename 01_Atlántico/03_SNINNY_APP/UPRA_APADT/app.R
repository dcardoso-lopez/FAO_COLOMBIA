...
# ---------- Rutas ----------
local_data_dir <- "C:/Users/Dell/Universidad de los andes/FAO-SAT - Documentos/General/06_Scripts/FAO_COLOMBIA/01_Atlántico/03_SNINNY_APP/UPRA_APADT/data"
...
# Asumimos formato ES: "," decimal y "." miles
...
# --- Cuartiles para el mapa (versión robusta, evita breaks duplicados) ---
...
# Nos quedamos solo con Atlántico
apadt_raw <- apadt_raw %>% dplyr::filter(DEPARTAMENTO_D == "ATLÁNTICO")
...
# ---------- Tibble APADT bruto (con parseo ES) ----------
apadt <- tibble(
  ...
  prop_apadt     = suppressWarnings(
    readr::parse_number(
      as.character(safe_pull(apadt_raw, col_prop_apadt)),
      locale = readr::locale(decimal_mark = ",", grouping_mark = ".")
    )
  )
)

# --- Detectar si las áreas vienen en m² y convertir a ha ---
max_area_mpio <- suppressWarnings(max(apadt$area_mpio_ha, na.rm = TRUE))
if (is.finite(max_area_mpio) && max_area_mpio > 2e7) {
  apadt$area_mpio_ha  <- apadt$area_mpio_ha  / 10000
  apadt$area_apadt_ha <- apadt$area_apadt_ha / 10000
}

# Si prop_apadt viene en % (>>1), normalizar a [0,1]
...
# ---------- Consolidar base a nivel MUNICIPIO-AÑO ----------
base_apadt <- apadt %>%
  left_join(pob, by = c("ano","COD_DANE_MUNI")) %>%
  mutate(
    prop_apadt = dplyr::coalesce(
      prop_apadt,
      if_else(area_mpio_ha > 0, area_apadt_ha / area_mpio_ha, NA_real_)
    )
  ) %>%
  group_by(
    ano,
  ...
  summarise(
    area_mpio_ha  = {
      x <- area_mpio_ha[is.finite(area_mpio_ha)]
      if (length(x) == 0) NA_real_ else max(x)
    },
    area_apadt_ha = {
      x <- area_apadt_ha[is.finite(area_apadt_ha)]
      if (length(x) == 0) NA_real_ else sum(x)
    },
    pob_total     = {
      x <- pob_total[is.finite(pob_total)]
      if (length(x) == 0) NA_real_ else sum(x)
    },
    .groups = "drop"
  ) %>%
  mutate(
    prop_apadt = dplyr::if_else(
      is.finite(area_mpio_ha) & area_mpio_ha > 0,
      area_apadt_ha / area_mpio_ha,
      NA_real_
    )
  )
      ...
      div(class="filters",
          div(class="filters-grid",
              ...
              div(class="filter",
                  ...
                  # el server pondrá Atlántico en el primer render
      ...
      div(class="content-grid",
          div(class="card viz-card viz-map",
              ...
              div(class="viz-body",
                  ...
                  div(class = "map-note",
                      "Nota: los puntos de corte mostrados en la leyenda se calcularon con cuartiles del indicador seleccionado."
                  )
              )
          ),
...
server <- function(input, output, session){
  ...
  # bandera para detectar primer render
  ...
  observeEvent(input$anio, {
    ...
    if (first_run()) {
      # primer render: dejamos Atlántico si existe
      default_sel <- if ("Atlántico" %in% deps_tc) "Atlántico" else "Todos"
      updateSelectInput(session, "f_dep",
                        choices  = c("Todos", deps_tc),
                        selected = default_sel)
      first_run(FALSE)
    } else {
      update_select_clean("f_dep", deps_tc, selected = input$f_dep)
    }
  ...
  observeEvent(input$btn_reset, {
    ...
    if (!is.null(input$f_prop))
      shinyWidgets::updateSliderTextInput(session, "f_prop", selected = c("0%","100%"))
    # al cambiar el año, el observer de año volverá a correr,
    # pero como first_run ya es FALSE, respetará el valor actual de f_dep
  })
  ...
  observe({
    ...
    if (is.null(input$f_dep) || input$f_dep == "Todos") {
      ...
      labels_legend <- vapply(
        seq_len(length(brks)-1),
        function(i){
          a <- brks[i]; b <- brks[i+1]
          if (ind == "prop_apadt") {
  ...
  observeEvent(input$map_apadt_shape_click, {
    if (is.null(input$f_dep) || input$f_dep == "Todos") {
      ...
      if (!is.na(nom_combo_tc) && nzchar(nom_combo_tc))
        updateSelectInput(session, "f_dep", selected = nom_combo_tc)
    }
  }, ignoreInit = TRUE)
  ...
  output$bar_top <- renderPlotly({
    ...
    if (ind == "prop_apadt") {
      ...
      hover_tpl   <- "%{customdata[0]}<br>%{y}<br>Porcentaje: %{x:.1%}<extra></extra>"
      xaxis_conf  <- list(
        title      = "Porcentaje",
        tickformat = ".0%",
        range      = c(0.20, 1.00)
      ...
      hover_tpl   <- "%{customdata[0]}<br>%{y}<br>Hectárea: %{x:,.0f} ha<extra></extra>"
      xaxis_conf  <- list(title = "Hectáreas")
    }
