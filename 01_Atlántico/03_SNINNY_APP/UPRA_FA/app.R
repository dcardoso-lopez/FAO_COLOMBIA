...
# ---------- Rutas ----------
local_data_dir <- "C:/Users/Dell/Universidad de los andes/FAO-SAT - Documentos/General/06_Scripts/FAO_COLOMBIA/01_Atlántico/03_SNINNY_APP/UPRA_FA/data"
...
format_breaks_list <- function(breaks, as_percent = FALSE){
  if (length(breaks) < 2) {
...
build_info_html <- function(breaks, as_percent){
  htmltools::HTML(sprintf(
    ...
    format_breaks_list(breaks, as_percent)
  ))
}
...
ui <- fluidPage(
  ...
  tags$head(
    ...
    ")),
    tags$script(HTML("
      Shiny.addCustomMessageHandler('updateQuantilesTooltip', function(message) {
        var el = document.getElementById('info_quantiles');
        if (!el) return;
...
server <- function(input, output, session){
  ...
  # bandera para fijar ATLÁNTICO solo en el primer render
  ...
  observeEvent(input$anio, {
    ...
    if (first_run()) {
      sel_dep <- if ("Atlántico" %in% deps_tc) "Atlántico" else "Todos"
      first_run(FALSE)
    } else {
  ...
  observe({
    ...
    if (is.null(input$f_dep) || input$f_dep == "Todos") {
      ...
      info_html <- build_info_html(brks, as_percent = (ind == "prop_fa"))
      session$sendCustomMessage("updateQuantilesTooltip", list(html = as.character(info_html)))
