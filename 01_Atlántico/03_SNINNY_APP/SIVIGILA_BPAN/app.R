...
# ---------- Rutas ----------
local_data_dir <- "C:/Users/Dell/Universidad de los andes/FAO-SAT - Documentos/General/06_Scripts/FAO_COLOMBIA/01_Atlántico/03_SNINNY_APP/SIVIGILA_BPAN/data"
...
bpan_raw <- bpan_raw %>% dplyr::filter(DEPARTAMENTO_O=="ATLÁNTICO")  # <--- solo Atlántico
...
# *** Lookup de departamentos y valor por defecto Atlántico ***
DEPS_ALL <- sort(unique(bpan_valid$DEP_O))
ATLANTICO_DEFAULT <- if ("Atlántico" %in% DEPS_ALL) "Atlántico" else (DEPS_ALL[1] %||% "Todos")
    ...
    div(
      ...
      div(
        ...
        div(
          ...
          selectInput(
            "f_depto_ocurr", NULL,
            # *** ahora usamos todos los deps disponibles y seleccionamos Atlántico por defecto
...
server <- function(input, output, session){
  ...
  observeEvent(input$btn_reset_bpan, {
    ...
    # *** reset dejando Atlántico seleccionado
  ...
  observeEvent(input$f_anio_bpan, {
    ...
    #     si no, tratamos de dejar Atlántico; si tampoco está, "Todos"
    sel_dep <- if (!is.null(input$f_depto_ocurr) && input$f_depto_ocurr %in% c("Todos", deps_o)) {
      input$f_depto_ocurr
    } else if (ATLANTICO_DEFAULT %in% deps_o) {
      ATLANTICO_DEFAULT
    } else {
      "Todos"
    }
