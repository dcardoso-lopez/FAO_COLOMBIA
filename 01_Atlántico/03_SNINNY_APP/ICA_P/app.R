...
# ---- Rutas / datos GOLDEN ----
golden_dir <- "C:/Users/Dell/Universidad de los andes/FAO-SAT - Documentos/General/06_Scripts/FAO_COLOMBIA/01_Atlántico/03_SNINNY_APP/ICA_P/data"
...
golden <- golden %>% dplyr::filter(DEPARTAMENTO_D=="Atlántico")
  ...
  div(
    ...
    div(
      ...
      div(
        ...
        # 2. Departamento (patrón con código inline)
        div(
          class = "filter-block",
          div(class="filter-label","¿En qué departamento?"),
          {
            ...
            # Default: Atlántico según el nombre
            idx_atl <- which(grepl("ATL", toupper(dpts$DEPARTAMENTO_D)))
            default_dep <- if (length(idx_atl)) dpts$DPTO2[idx_atl[1]] else "Todos"
